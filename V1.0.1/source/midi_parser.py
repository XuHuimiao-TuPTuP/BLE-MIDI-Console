...
def describe_message(data: bytes) -> str:
    ...
    if status == 0xF1 and len(data) >= 2:
        piece = (data[1] >> 4) & 0x07
        return f"MTC Quarter Frame  piece={piece} value={data[1] & 0x0F}"
    if status == 0xF2 and len(data) >= 3:
        position = data[1] | (data[2] << 7)
        return f"Song Position Pointer  MIDI-beats={position} (每拍 6 clocks)"
    if status == 0xF3 and len(data) >= 2:
        return f"Song Select  song={data[1]}"
    system = {
...
def describe_sysex(data: bytes) -> str:
    ...
    if len(body) >= 4 and body[0] == 0x7E and body[2:4] == bytes((0x06, 0x01)):
        return f"SysEx · Universal Identity Request, device=0x{body[1]:02X}" + suffix
    if len(body) >= 4 and body[0] == 0x7E and body[2:4] == bytes((0x06, 0x02)):
        return f"SysEx · Universal Identity Reply, device=0x{body[1]:02X}" + suffix
    if len(body) >= 6 and body[0] == 0x7F and body[2:4] == bytes((0x04, 0x01)):
        volume = body[4] | (body[5] << 7)
        return f"SysEx · Universal Master Volume, device=0x{body[1]:02X}, value={volume}/16383" + suffix
    if len(body) >= 6 and body[0] == 0x7F and body[2:4] == bytes((0x04, 0x02)):
        balance = body[4] | (body[5] << 7)
        return f"SysEx · Universal Master Balance, device=0x{body[1]:02X}, value={balance - 8192}" + suffix
    if len(body) >= 4 and body[0] == 0x7F and body[2] == 0x06:
        mmc = {0x01: "Stop", 0x02: "Play", 0x03: "Deferred Play", 0x04: "Fast Forward",
               0x05: "Rewind", 0x06: "Record Strobe", 0x07: "Record Exit", 0x09: "Pause"}
        return f"SysEx · MIDI Machine Control {mmc.get(body[3], f'command 0x{body[3]:02X}')}" + suffix
