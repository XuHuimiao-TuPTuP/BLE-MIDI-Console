...
class App(tk.Tk):
    ...
    def _style(self) -> None:
        ...
        style.configure("TButton", background="#243444", foreground=INK, padding=(12, 7), borderwidth=0)
        style.map("TButton", background=[("active", "#31506b"), ("disabled", "#1d2731")])
        style.configure("Accent.TButton", background=ACCENT, foreground="white")
        style.map("Accent.TButton", background=[("active", "#54adf6")])
        style.configure("Treeview", background="#111923", fieldbackground="#111923", foreground=INK, rowheight=27, borderwidth=0)
        style.configure("Treeview.Heading", background="#22303e", foreground=INK, relief="flat", font=("Segoe UI Semibold", 9))
        style.map("Treeview", background=[("selected", "#24567c")])
        style.configure("TNotebook", background=BG, borderwidth=0)
        style.configure("TNotebook.Tab", background="#1b2733", foreground=MUTED, padding=(14, 8))
        style.map("TNotebook.Tab", background=[("selected", "#29435a")], foreground=[("selected", "white")])
        style.configure("TEntry", fieldbackground="#101923", foreground=INK, insertcolor=INK)
    ...
    def _build_logs(self, parent) -> None:
        ...
        self.logs = {}
        for key, label in (("all", "全部"), ("rx", "RX 接收"), ("tx", "TX 发送"), ("raw", "原始 BLE")):
    ...
    def _text(self, parent, height=None):
        return tk.Text(parent, height=height, bg="#0d141c", fg=INK, insertbackground=INK,
                       selectbackground="#275579", relief="flat", padx=9, pady=8,
